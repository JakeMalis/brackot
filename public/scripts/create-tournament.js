function personalizeElements() {
  ...
  games.forEach((game) => {
    gameFileName = game
      ...
      .replace(/\./g, "")
      .replace(/\'/g, "");
    $("#createTournamentGameCarousel").append(
      '<label id="create' +
        gameFileName +
        'TournamentLabel" onclick="animateGameCarousel(this.id)" class="createTournamentGamesLabel"><input name="newTournamentGame" class="createTournamentGamesRadio" type="radio" value="' +
        game +
        '""></input><picture><source srcset="../media/game_images/' +
        gameFileName +
        '.webp" type="image/webp"><img class="createTournamentGamesImage" src="../media/game_images/' +
        gameFileName +
        '.jpg"></picture></label>'
    );
  });

  $('[name="newTournamentDate"]').attr('placeholder', yyyy+'-'+mm+'-'+dd);
  $('[name="newTournamentDate"]').flatpickr();
  $('[name="newTournamentStartTime"]').timepicker({'timeFormat': 'H:i'});
  $('#bracketType').niceSelect();
}
...
function searchGameCreateTournament(searchbar) {
  var value = $(searchbar)
    ...
    .toLowerCase()
    .replace(/ /g, "")
    .replace(/\'/g, ""); // value stores the current string in the searchbar, in lowercase and with no spaces
...
function addTournament() {
  try {
    if (
      ...
      document.querySelector('input[name="newTournamentGame"]:checked').value ==
        "" ||
      null ||
      undefined ||
      document.querySelector(
        'input[name="newTournamentParticipantType"]:checked'
      ).value == "" ||
      null ||
      undefined
    )
    ...
    firebase
      ...
      .add({
        ...
        game: document.querySelector('input[name="newTournamentGame"]:checked')
          .value,
      ...
      .then(function () {
        ...
        if (
          document.getElementById("alertBox").classList.contains("errorAlert")
        ) {
          document.getElementById("alertBox").classList.remove("errorAlert");
        }
        document.getElementById("alertTextBold").innerHTML = "Update: ";
        document.getElementById("alertText").innerHTML =
          "Your tournament has been added!";
      })
      .then(() => {})
    ...
    document.getElementById("alertText").innerHTML =
      "Your tournament submission was incomplete. All of the required fields must be complete before submitting.";
    alert(error);
  }
}
